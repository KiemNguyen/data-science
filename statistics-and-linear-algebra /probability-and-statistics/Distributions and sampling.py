...
lowest_median_income_row = income["median_income"].idxmin()
# County with the lowest median income in the US
lowest_income_county = income["county"][lowest_median_income_row]

high_pop = income[income["pop_over_25"] > 500000]
high_pop_lowest_median_income_row = high_pop["median_income"].idxmin()
# County that has more than 500000 residents with the lowest median income
lowest_income_high_pop_county = high_pop["county"][high_pop_lowest_median_income_row]
...
# Standard deviations away from the mean of large_sample .18
mean = numpy.mean(large_sample)
deviations_from_mean = abs(.18 - mean) / large_sample_std
...
ratios_count = len([i for i in mean_ratios if i >= .675])
significance_value = ratios_count / len(mean_ratios)